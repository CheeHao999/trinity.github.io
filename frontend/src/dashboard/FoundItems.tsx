...
import { FoundItem } from '../shared/types';
import { MapPin, Calendar, Phone, Plus, Search, CheckCircle, Trash2 } from 'lucide-react';
...
const FoundItems: React.FC = () => {
  ...
  const fetchItems = async () => {
    try {
      const response = await client.get('/found-items');
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await client.delete(`/found-items/${id}`);
  ...
  const handleResolve = async (id: string) => {
    ...
    try {
      await client.put(`/found-items/${id}`, { status: 'RESOLVED' });
  ...
  const handleClaim = async (id: string) => {
    if (!window.confirm('Do you want to claim this item? Admins will be notified.')) return;
    try {
      await client.post(`/found-items/${id}/claim`);
      alert('Claim request sent successfully! An admin will review it.');
    } catch (error) {
  ...
  return (
    <div className="animate-fade-in">
      <div className="flex flex-col md:flex-row justify-between items-end mb-12 border-b-2 border-white pb-8 gap-6">
        <div>
          <h1 className="text-6xl md:text-8xl font-black uppercase tracking-tighter leading-none mb-2 text-white">Found</h1>
          <p className="text-gray-500 font-bold uppercase tracking-widest text-sm">Items reported found</p>
        </div>
        <Link
          to="/create-found"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredItems.map((item) => (
            <div key={item.id} className="group border-2 border-white bg-surface hover:bg-white transition-colors duration-300 relative">
              {item.matchedLostItemId && (
                <div className="absolute top-4 left-4 bg-white text-black text-xs px-3 py-1 font-bold uppercase tracking-widest border border-black z-10">
                  Match Found
                </div>
              )}
              ...
              {isAdmin && (
                <div className="relative overflow-hidden aspect-video border-b-2 border-white">
                  ...
                  {user && user.id !== item.userId && item.status !== 'RESOLVED' && (
                    <div className="absolute top-2 right-2 z-10">
                      <button
                        ...
                        className="bg-yellow-500 text-black font-bold uppercase tracking-wider text-xs px-3 py-2 rounded hover:bg-yellow-400 transition-colors shadow-lg"
                      >
                        Claim This
                      </button>
                    </div>
                  ...
                  <>
                    ...
                    <div className="space-y-3 border-t-2 border-gray-800 group-hover:border-gray-200 pt-4">
                      <div className="flex items-center gap-3 text-sm font-bold uppercase tracking-wide text-gray-500 group-hover:text-gray-800">
                        <Calendar size={16} />
                        <span>{new Date(item.foundDate || item.date).toLocaleDateString()}</span>
                      </div>
                    </div>
                  </>
                ) : (
